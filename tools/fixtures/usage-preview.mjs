...
export function previewUsage(mode='offpeak',now=Date.now(),extraOutputTokens=0,tariff=mode) {
    const previewTime=Date.parse(tariff==='peak'?'2026-09-11T10:30:00+08:00':'2026-09-11T20:30:00+08:00');
    ...
    if(mode!=='empty' && extraOutputTokens>0)events.push({seq:4,type:'assistant/message',time:Date.parse('2026-09-11T08:30:00+08:00'),data:{turn:1,step:4,usage:{inputTokens:0,outputTokens:extraOutputTokens}}});
    ...
    const cost=sessionCost(events,0,PRICING,previewTime), child=sessionCost(mode==='empty'?[]:[events[0],events[1]],0,PRICING,previewTime);
    return {...cost,selfTotal:cost.totals.total-child.totals.total,children:{count:mode==='empty'?0:1,total:child.totals.total,requests:child.requests,unavailable:0},sessionId:'preview-session',activity,preview:true,previewTime,updatedAt:now};
}
