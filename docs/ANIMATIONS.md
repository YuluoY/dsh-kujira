...
26 段导入素材曾为保留宽幅特效而整体缩小并增加透明留白，导致播放时人物变小。`animationFraming` 从每段导入滤镜推导逆变换，恢复人物原始坐标与落脚基线，保留宽幅特效；原有视频文件无需重编码。导入脚本同步生成校正参数，播放器只在切换片段时设置，普通片段恢复默认值，两个视频缓冲区独立维护。点击与拖拽由固定的 stage 区域处理，扩展到区域外的特效不拦截页面操作。动画本身的蹲坐、跳跃、远近运动仍按原画播放，不逐帧强行拉伸姿态。

思考泡参考了 [Summer 的 Thought bubble](https://lottiefiles.com/free-animation/thought-bubble-v6uqbLyQUT) 与 [Ethan Barnowsky 的 Thought Bubble](https://giphy.com/stickers/face-accessories-l378srTrjHN2cpSYE) 的三段思考泡视觉方向。实现为项目自己的 CSS 形状和动效，不引入图片、GIF 或 Lottie 播放依赖。按用户截图还原青绿色填充、粗黑轮廓、多瓣云朵、两个正圆尾泡、渐进膨胀与延迟文字；尾迹预留 72px，仍在浏览器 top layer 绘制。

本地预览中的「气泡与人物比例」打开 `/companion-lab.html`，可输入文字、重播/收起气泡、选择受影响动作，并切换「对比旧缩放」。该页面调用实际 BubbleLayer、fitBubble 和视频播放器；不使用账户数据或创建任务。

轮廓对照：主泡采用独立 SVG 多瓣云朵（380:320 比例），并按人物所在一侧镜像轮廓；文字不镜像。颜色直接从用户截图采样：主泡 #77ded0、浅色描边 #191919（深色背景使用 #303b38 并加轻微边缘对比）、中圆泡 #52a79b、小圆泡 #438d82。主泡宽度按人物 stage 的 74% 计算，限制在 144–208px；默认 260px 人物对应约 192 × 162px，圆泡直径 28/18px，描边随主泡比例缩放。小视口同时按宽度和可用高度限制主泡尺寸。「只看轮廓」在预览中隐藏文字并切到白底，便于与截图比较；生产气泡保留文字。

悬浮间距：默认上/下方布局中，圆泡间距 12px，最小圆泡距估算头部边界 13px；侧面布局保留 8px。放置算法保护面部区域，并整体检查主泡和两圆泡的屏幕边界，按空间选上方、侧面或下方。位置被边缘约束时重新定位尾迹，避免套用固定相对位置造成过大间隔。头部边界根据统一 stage 坐标估算，素材自身的跳跃或姿态变化不触发逐帧位置追踪。
