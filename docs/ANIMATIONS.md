...
人物说话使用椭圆思考气泡：小圆泡、中圆泡、主体依次出现（0 / 160 / 360 ms），主体用轻微挤压与回弹舒展 620 ms，文字延迟到 660 ms 渐入；退场约 340 ms，顺序相反。减少动态效果时直接呈现。进展气泡保留紧凑信息样式，两种内容共用一个 top-layer 容器，紧急进展优先；面板或菜单打开时收起。放置时比较上下左右空间，限制正文和圆点在视口内，避免人物与滚动容器遮盖。
...
思考泡参考了 [Summer 的 Thought bubble](https://lottiefiles.com/free-animation/thought-bubble-v6uqbLyQUT) 与 [Ethan Barnowsky 的 Thought Bubble](https://giphy.com/stickers/face-accessories-l378srTrjHN2cpSYE) 的三段思考泡视觉方向。实现为项目自己的 CSS 形状和动效，不引入图片、GIF 或 Lottie 播放依赖。纸白填充、蓝色轮廓、非对称椭圆、渐进膨胀与延迟文字；尾迹预留 62px，仍在浏览器 top layer 绘制。
