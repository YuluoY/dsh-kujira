...
Updates align separately installed global official components used by DSH and repair incompatible shared dependencies. Matching releases are checked before installation, and the actual dependency graph is checked afterward. If the package manager still resolves an old indirect dependency, the updater makes one targeted repair attempt before reporting failure. Unrelated global tools are preserved. A mixed installation exposes **Repair current version**, which repairs the same release and still requires a restart. Plugins installed separately in a DSH profile must be updated through DSH's plugin manager.

Each installation command has a ten-minute timeout and a per-user process lock. Exiting DSH cancels the installer. After success, restart DSH normally to run the installed version; the current running version stays visible until then. Settings persist in `harness-update.json` in the plugin data directory. `harnessUpdate.enabled: false` disables this service. `PREVIEW_UPDATE=1` enables read-only version checks in previews; previews can never install updates.
