...
export function parseOfficialPricing(html) {
    ...
    const knownSchedule = /高峰时段为北京时间周一至周五/.test(text) || /北京时间周一至周五（不含中国法定节假日）\s*9:00\s*[-–]\s*12:00、14:00\s*[-–]\s*18:00\s*为高峰时段/.test(text);
    if (!knownSchedule || !/9:00\s*[-–~至]\s*12:00/.test(text) || !/14:00\s*[-–~至]\s*18:00/.test(text)) throw new Error('schedule-changed');
