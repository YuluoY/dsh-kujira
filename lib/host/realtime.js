...
export function createRealtime({ getKey, fetch: request = globalThis.fetch, cacheDir = join(homedir(), '.dsh', 'dsh-kujira'), disabled = false } = {}) {
    let pricing = structuredClone(PRICING), settings = { automatic: false, modelAssist: false, intervalHours: 24, model: 'deepseek-flash' }, checkedAt = 0, attemptedAt = 0, modelAt = 0, summary = '', busy = null;
    ...
    const ready = disabled ? Promise.resolve() : readFile(path,'utf8').then(raw => {
        ...
        settings = { ...settings, ...v.settings }; checkedAt = Number(v.checkedAt)||0; attemptedAt=Number(v.attemptedAt)||0; modelAt=Number(v.modelAt)||0; summary=String(v.summary||'');
    }).catch(()=>{});
    ...
    return { ready, refresh, status, pricing: () => pricing,
        async configure(value) { await ready; if (busy) return { ...status(), ok:false, message:'正在同步，请稍后修改设置' }; for (const key of ['automatic','modelAssist']) if (typeof value[key] === 'boolean') settings[key] = value[key]; if (typeof value.model === 'string' && /^deepseek-[a-z0-9-]{1,70}$/.test(value.model)) settings.model = value.model; if ([6,12,24,72].includes(value.intervalHours)) settings.intervalHours = value.intervalHours; await save(); return status(); } };
}
