...
export function createActivityReader(getSessions) {
    const completed=new Map(), cache=new WeakMap(), snapshots=new WeakMap();const epoch=randomUUID();let revision=0;
    const fold=session=>{
    ...
    const read=sessionId=>{
        ...
        if(old?.activity===activity && versions.length===old.versions.length && versions.every((v,i)=>v===old.versions[i]))return old.response;
        const response={ok:true,sessionId,activity:{...activity,children},epoch,revision:++revision,updatedAt:Date.now()};
        snapshots.set(session,{activity,versions,response});return response;
    };
