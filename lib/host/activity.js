...
export function createActivityReader(getSessions) {
    ...
    const read=sessionId=>{
        ...
        const children=(activity.catalog || activity.children).flatMap(child=>{
            ...
            return [{...child,stage:value?.stage || 'unknown',summary:value?.summary || '',current:value?.current || null,operations:value?.operations || [],endedAt:value?.endedAt || null,updatedAt:value?.updatedAt || null}];
        });
