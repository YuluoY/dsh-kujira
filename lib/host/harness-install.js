 ...
 */
export function installCommand(installation, version) {
  ...
  const args = installation.manager === "pnpm"
    ? ["add", "--global", `${HARNESS_PACKAGE}@${version}`, `--registry=${NPM_REGISTRY}`, "--config.engine-strict=true", "--reporter=append-only"]
    : ["install", "--global", `${HARNESS_PACKAGE}@${version}`, `--registry=${NPM_REGISTRY}`, "--engine-strict", "--no-audit", "--no-fund"];
