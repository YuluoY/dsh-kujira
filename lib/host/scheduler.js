...
export function createPeakScheduler({
  ...
  const tick = () => {
    if (disposed || !available || !enabled || !peak())
      for (const waiter of [...waiting.values()]) waiter.release();
  };
  ...
  const snapshot = () => ({
    ...
    available,
    preview,
    rate: rateAt(now(), getConfig()),
    nextAt:
      nextSwitch(now(), getConfig()).ms == null
        ? null
        : now() + nextSwitch(now(), getConfig()).ms,
    pausing:
      enabled && available && peak()
        ? getAgents().filter(
            (agent) =>
              agent.status === "running" &&
              !paused(agent.id || agent.session?.id),
          ).length
        : 0,
      ...
      await new Promise((resolve, reject) => {
        const token = {},
          id =
            ...
            payload.agent?.session?.header?.id ||
            payload.agent?.id;
        const remove = () => {
