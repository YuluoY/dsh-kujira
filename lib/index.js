...
export function apply(ctx, config)
{
    ...
    const scheduler=createPeakScheduler({directory:config?.scheduler?.directory || config?.inventory?.directory || join(homedir(),'.dsh','dsh-kujira'),getConfig:()=>billingConfig || realtime.pricing(),preview:!!config?.scheduler?.preview,now:config?.scheduler?.now || Date.now});
    ctx.inject?.(['agents'],scope=>{scheduler.setAvailable(!!scope.agents);scope.effect(()=>()=>scheduler.setAvailable(false));});
    ctx.effect(()=>ctx.on('agent/pre-step',(payload,next)=>scheduler.gate(payload,next),{prepend:true}));
    ctx.effect(()=>()=>scheduler.dispose());
    const activityReader = createActivityReader(() => sessions);
    ...
    const queryUsage = createUsageReader(() => sessions, () => billingConfig || realtime.pricing());
    ...
    const inventory = createInventory({directory:config?.inventory?.directory,getConfig:()=>billingConfig || realtime.pricing(),enabled:()=>growthEnabled,isBusy:()=>['thinking','working','result'].includes(watch.snapshot().state)});
    ctx.effect(()=>ctx.on('session/event',(session,event)=>{
        if(['assistant/message','assistant/attempt','turn/end'].includes(event?.type)) {
            ...
            if(live)inventory.observe(live).catch(error=>console.warn('[dsh-kujira] Inventory persistence failed:',error.code || error.message));
        }
    }));
 ...
 res.statusCode=200;res.setHeader('Content-Type','text/css; charset=utf-8');res.setHeader('Cache-Control','no-store');res.end(styles.join('\n'));
                ...
                if (rest === 'inventory') {
                    if (req.method === 'GET') { sendJson(res, 200, await inventory.snapshot()); return; }
                    if (req.method !== 'POST') { sendJson(res,405,{ok:false}); return; }
                ...
                if (rest === 'activity') {
                    ...
                    const value=id?activityReader.read(id):{ok:false,message:'缺少会话 ID'};
                    if(value.ok && scheduler.paused(id))value.activity={...value.activity,stage:'paused',phase:'waiting',label:'峰价暂停',attention:null};
