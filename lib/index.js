...
export function apply(ctx, config)
{
 ...
 res.statusCode=200;res.setHeader('Content-Type','text/css; charset=utf-8');res.setHeader('Cache-Control','no-store');res.end(styles.join('\n'));
                ...
                if (rest === 'inventory') {
                    ...
                    const result=value.action==='mode'?await inventory.configure(value.free):value.action==='rules'?await inventory.configureRules(value.rules,value.revision):await inventory.consume(value.kind,value.requestId);
                ...
                if (rest === 'activity') {
                    ...
                    if (url.searchParams.has('messagesBefore')) {
                        const value=activityReader.messages(id, Number(url.searchParams.get('messagesBefore')));
                        sendJson(res,value.ok?200:400,value);return;
                    }
                ...
                if (rest === 'usage')
                {
                    ...
                    sendJson(res, sessionId ? 200 : 400, sessionId ? await queryUsage.tree(sessionId)
