import { createDailyUsage } from "./host/daily-usage.js";
import { createHarnessUpdater } from "./host/harness-update.js";
import { createHolidayCalendar } from "./host/holidays.js";
...
export function apply(ctx, config) {
  ...
  const holidays = createHolidayCalendar({
    directory: config?.holidayCalendar?.directory || config?.inventory?.directory,
    disabled: config?.holidayCalendar?.enabled === false,
  });
  let pricingSnapshot;
  const getPricing = () => {
    const prices = billingConfig || realtime.pricing();
    const calendar = holidays.calendar();
    if (pricingSnapshot?.prices !== prices || pricingSnapshot?.calendar !== calendar)
      pricingSnapshot = { prices, calendar, value: { ...prices, holidayCalendar: calendar } };
    return pricingSnapshot.value;
  };
  ctx.effect(() => {
    holidays.refresh();
    const timer = setInterval(() => holidays.refresh(), 3600000);
    timer.unref?.();
    return () => { clearInterval(timer); holidays.dispose(); };
  });
  ctx.effect(() => {
    let disposed = false;
    realtime.ready.then(() => {
  ...
  const harnessUpdater = createHarnessUpdater({
    ...
    disabled: config?.harnessUpdate?.enabled === false,
    preview: !!config?.harnessUpdate?.preview,
    activeTasks: () => agents?.list ? agents.list().filter((agent) => agent.status === "running").length : null,
  });
  ctx.effect(() => {
    harnessUpdater.check();
    const timer = setInterval(() => harnessUpdater.check(), 3600000);
    timer.unref?.();
    return () => { clearInterval(timer); harnessUpdater.dispose(); };
  });
  ...
  const scheduler = createPeakScheduler({
    ...
    getGoal: (agent) => ctx.get?.("goals")?.get(agent),
    getTitle: (agent) => ctx.get?.("sessionProjections")?.stateOf(agent?.session, "title"),
    directory:
      config?.scheduler?.directory ||
      config?.inventory?.directory ||
      join(homedir(), ".dsh", "dsh-kujira"),
    getConfig: getPricing,
  ...
  ctx.effect(() =>
    ctx.on("agent/pre-step", (payload, next) => harnessUpdater.gate(payload, () => scheduler.gate(payload, next)), {
      prepend: true,
    }),
  );
  ctx.effect(() =>
    ctx.on(
      "agent/request",
      (payload, next) => harnessUpdater.gate(payload, () => scheduler.gate(payload, next, "request")),
      {
        prepend: true,
      },
    ),
  ...
  const exchange = createExchangeClient({
    disabled: config?.exchangeRates?.enabled === false,
  });
  ...
  const dailyUsage = createDailyUsage({
