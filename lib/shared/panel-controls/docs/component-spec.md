...
思考气泡保持两个圆泡和一个主体，共三个图形，入场延迟 0/160/360ms、文字延迟 660ms，退出反向 340ms。主泡改为用户截图中的青绿色多瓣 SVG 云朵与粗黑描边，尾泡为正圆；颜色集中在 surface-tokens.css 的独立思考泡 token。布局预留 72px 尾迹，减少动态效果时直接显示内容。

视频视觉平面可根据导入时的缩小与留白进行逆变换，但交互平面保持原 stage 大小。变换只在切换视频时设置，不按帧测量 DOM 或像素。26 段配置均由滤镜推导并有坐标逆变换测试；普通素材与无效配置恢复 1/1/0，防止前后缓冲复用遗留样式。

视觉纠偏：仅参考出现顺序不能宣称还原参考效果。对照用户截图时必须同时核对外轮廓（云朵而非椭圆）、描边重量、采样色值、尾泡圆度与比例，再验收动效。预览的「只看轮廓」用于白底静态对照；带文字模式检查内容可读性。

尺寸/间距跟进：主泡按人物宽度 0.74 联动，144–208px 限幅，默认约 192 × 162px；尾泡 28/18px。上/下布局对估算头部保留 13px，圆泡间保留 12px；侧面布局保留 8px。面部碰撞代价与画面边界一起决定位置，尾泡坐标由同一次计算产生。默认文字 12px、深色主题描边用 charcoal 并有轻微外沿对比，保留薄荷绿主体。
