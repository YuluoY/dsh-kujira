/**
 * ============================================================================
 * lib/shared/billing.js —— 三桶计费与峰谷选档（纯函数，零依赖）
 * ============================================================================
 *
 * 【为什么要分三桶】
...
const BEIJING_FORMAT = new Intl.DateTimeFormat('en-US', {
    timeZone: 'Asia/Shanghai', hour12: false, weekday: 'short', hour: '2-digit', minute: '2-digit'
});
 ...
 * 判断某个时刻是高峰还是空闲。
 *
 * 默认规则（DeepSeek 官方）：北京时间**工作日** 9:00–12:00、14:00–18:00 为高峰，
 * 其余（含周末全天）为空闲。
 *
 * @param ts 毫秒时间戳
...
export function rateAt(ts, cfg)
{
    ...
    if (workdays.indexOf(t.weekday) < 0)
    {
        return 'offpeak';
    }
...
export function nextSwitch(ts, cfg)
{
    ...
    // Cover a full weekend. Probe exact minute boundaries rather than preserving seconds.
    const STEP = 60 * 1000;
