...
export function sessionCost(events, inherited = 0, config = PRICING, now = Date.now()) {
    const { rows, reported, route } = usageRecords(events, inherited, config);
 ...
 */
export function usageRecords(events, inherited = 0, config = PRICING) {
    const rows = new Map();
    ...
    for (let index = 0; index < events.length; index++) {
        ...
        rows.set(key, { key, seq: event.seq ?? index, turn: d.turn, model, provider, ts, usage, result });
    }
    return {rows, reported, route};
}
