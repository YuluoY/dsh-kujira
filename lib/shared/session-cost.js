...
export function sessionCost(events, inherited = 0, config = PRICING, now = Date.now()) {
    return costFromRecords(usageRecords(events, inherited, config), config, now, inherited);
}
 ...
 */
export function costFromRecords({rows,reported,route},config=PRICING,now=Date.now(),inherited=0) {
 ...
 */
export function usageRecords(events, inherited = 0, config = PRICING, previous = null) {
    let rows = previous?.rows || new Map();
    let children = previous?.children || new Set();
    const changes = new Map();
    let route = previous?.route || {}, attempt = previous?.attempt || 0, reported = previous?.reported || 0;
    for (let index = previous?.offset || 0; index < events.length; index++) {
        ...
        if (event.type === 'subagent/catalog' && typeof d.childId === 'string' && ['one-shot','continuable'].includes(d.mode)) {
            if (children === previous?.children) children = new Set(children);
        ...
        const key = Array.isArray(d.stream) && Number.isSafeInteger(event.seq) ? `settlement:${event.seq}` : Number.isInteger(d.turn) && Number.isInteger(d.step) ? `${d.turn}:${d.step}:${attempt}` : `seq:${event.seq ?? index}`;
        ...
        const result = !usage ? { ok: false, reason: 'invalid-usage' }
            ...
            : !isDeepSeekProvider(provider) ? { ok: false, reason: 'unsupported-provider' }
            : costOf(usage, model, ts, config);
        const row = { key, seq: event.seq ?? index, turn: d.turn, model, provider, ts, usage, result };
        if (rows === previous?.rows) rows = new Map(rows);
        rows.set(key, row); changes.set(key, row);
    }
    return {rows, children, reported, route, attempt, offset:events.length, changes};
}
