 ...
 * @param {number} since Earliest eligible usage timestamp, after settlement deduplication.
 * @returns {object} Session costs, pricing coverage and tariff status.
 */
export function sessionCost(events, inherited = 0, config = PRICING, now = Date.now(), since = -Infinity) {
    const rows = new Map();
