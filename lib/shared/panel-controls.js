import {createSearchSelect} from "./client/search-select.js";
...
export function createControls(React) {
    ...
    function NumberField({label, value, onChange, min=0, max=Infinity, step=1, disabled=false, integer=false, unit="", prefix=false}) {
        ...
        return h('div',{className:'kj-number'},
            ...
            h('input',{ref:input,type:'text',inputMode:integer?'numeric':'decimal',role:'spinbutton','aria-label':label,'aria-valuemin':min,'aria-valuemax':Number.isFinite(max)?max:undefined,'aria-valuenow':value,'aria-valuetext':unit?(prefix?t(unit)+' '+draft:draft+' '+t(unit)):undefined,value:draft,disabled,onKeyDown:e=>{if(e.key==='ArrowUp'||e.key==='ArrowDown'){e.preventDefault();nudge(e.key==='ArrowUp'?1:-1);}if(e.key==='Enter')e.currentTarget.blur();},onChange:e=>{setDraft(e.currentTarget.value);const n=parseNumber(e.currentTarget.value);if(Number.isFinite(n)&&n>=min)onChange(clampValue(n));},onBlur:()=>{const n=parseNumber(draft);const next=Number.isFinite(n)?clampValue(n):value;setDraft(formatted(next));onChange(next);}}),!prefix&&unit?h('span',{className:'kj-number-unit','aria-hidden':true},t(unit)):null),
    ...
    return {Nickname,HelpLabel,Disclosure,Select,SearchSelect,NumberField,Range,Skeleton,TooltipHost,beginRefresh,finishRefresh};
}
