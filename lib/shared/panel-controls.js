...
export function createControls(React) {
    ...
    function Range({label, labelNode, value: savedValue, min, max, unit='', onChange, commitOnRelease=false}) {
        const input=useRef(null), tip=useRef(null), id=useId();
        const gesture=useRef(false), pending=useRef(savedValue), change=useRef(onChange);
        change.current=onChange;
        const [draft,setDraft]=useState(savedValue);
        const [hover,setHover]=useState(false), [focus,setFocus]=useState(false), [drag,setDrag]=useState(false);
        const value=commitOnRelease&&drag?draft:savedValue;
        const end=(cancel=false)=>{if(!gesture.current)return;gesture.current=false;setDrag(false);if(commitOnRelease&&!cancel&&pending.current!==savedValue)change.current(pending.current);};
        const update=e=>{const next=normalizeNumber(Number(e.currentTarget.value),{min,max,step:1,integer:true});if(next===null)return;pending.current=next;if(commitOnRelease&&gesture.current)setDraft(next);else onChange(next);};
        const ratio=Math.max(0,Math.min(1,(value-min)/(max-min)));
        const visible=hover||focus||drag;
        useEffect(()=>{if(!drag)return;const up=()=>end(),cancel=()=>end(true);window.addEventListener('pointerup',up);window.addEventListener('pointercancel',cancel);return()=>{window.removeEventListener('pointerup',up);window.removeEventListener('pointercancel',cancel);};},[drag]);
        useLayoutEffect(()=>{
            if(!visible)return;
        ...
        return h('div',{className:'kj-slider-row'}, labelNode || h('span',null,label),
            h('input',{ref:input,className:'kj-slider',type:'range',min,max,value,'aria-label':label,'aria-valuetext':(unit==='%' ? number(value/100,{style:'percent',maximumFractionDigits:0}) : number(value)+unit),'aria-describedby':visible?id:undefined,
                style:{'--kj-progress':ratio*100+'%'},onChange:update,onPointerEnter:()=>setHover(true),onPointerLeave:()=>setHover(false),onFocus:()=>setFocus(true),onBlur:()=>{setFocus(false);end();},onPointerDown:()=>{gesture.current=true;pending.current=savedValue;setDraft(savedValue);setDrag(true);},onPointerUp:()=>end(),onPointerCancel:()=>end(true),onKeyDown:e=>{if(e.key==='Escape'&&gesture.current){e.preventDefault();e.stopPropagation();end(true);}}}),
