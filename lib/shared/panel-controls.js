...
/** Small, dependency-free React controls for the DSH companion. */
...
export function createControls(React) {
    ...
    /** Select-only combobox. Focus stays on trigger; active option is announced. */
    ...
    function NumberField({label, value, onChange, min=0, max=Infinity, step=1, disabled=false}) {
        ...
        return h('div',{className:'kj-number'},
            ...
            h('input',{ref:input,type:'text',inputMode:'decimal',role:'spinbutton','aria-label':label,'aria-valuemin':min,'aria-valuemax':Number.isFinite(max)?max:undefined,'aria-valuenow':value,value:draft,disabled,onKeyDown:e=>{if(e.key==='ArrowUp'||e.key==='ArrowDown'){e.preventDefault();nudge(e.key==='ArrowUp'?1:-1);}if(e.key==='Enter')e.currentTarget.blur();},onChange:e=>{setDraft(e.currentTarget.value);const n=parseNumber(e.currentTarget.value);if(Number.isFinite(n)&&n>=min)onChange(Math.min(max,n));},onBlur:()=>{const n=parseNumber(draft);const next=Number.isFinite(n)?Math.min(max,Math.max(min,n)):value;setDraft(formatted(next));onChange(next);}}),
    ...
    /** Shaped placeholders; the outer panel owns the stable viewport dimensions. */
    ...
    /** Delegated tooltip, in the top layer so panel scrolling cannot clip it. */
    function TooltipHost({rootRef}) {
        const tooltip=useRef(null), id=useId();
        useEffect(()=>{
            const root=rootRef.current, el=tooltip.current;
            let timer=0, target=null;
            const hide=()=>{clearTimeout(timer);if(target)target.removeAttribute('aria-describedby');target=null;if(el.matches(':popover-open'))el.hidePopover();};
            const show=(node,immediate=false)=>{
                if(node===target)return;
                hide();target=node;
                const render=()=>{
                    if(!node.isConnected){hide();return;}
                    el.textContent=node.dataset.tooltip;
            ...
            const enter=e=>{if(e.type==='focusin'&&!e.target.matches(':focus-visible'))return;const node=e.target.closest?.('[data-tooltip]');if(node&&root.contains(node))show(node,e.type==='focusin');};
            ...
            const leave=e=>{if(target&&target.contains(e.target)&&!target.contains(e.relatedTarget))hide();};
            const escape=e=>{if(e.key==='Escape')hide();};
            const observer=new MutationObserver(()=>{if(target&&(!target.isConnected||(target.closest('.dsh-kujira-orbs')&&root.dataset.orb!=='1')))hide();});
            observer.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:['data-orb']});
            ...
            root.addEventListener('pointerout',leave);root.addEventListener('focusout',leave);root.addEventListener('pointerdown',hide);
            document.addEventListener('keydown',escape);window.addEventListener('resize',hide);document.addEventListener('scroll',hide,true);
            return()=>{observer.disconnect();hide();root.removeEventListener('click',click);document.removeEventListener('pointerdown',outside);root.removeEventListener('pointerover',enter);root.removeEventListener('focusin',enter);root.removeEventListener('pointerout',leave);root.removeEventListener('focusout',leave);root.removeEventListener('pointerdown',hide);document.removeEventListener('keydown',escape);window.removeEventListener('resize',hide);document.removeEventListener('scroll',hide,true);};
        },[rootRef,id]);
    ...
    return {HelpLabel,Disclosure,Select,NumberField,Range,Skeleton,TooltipHost,beginRefresh,finishRefresh};
}
