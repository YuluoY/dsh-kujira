...
export function createUsageComponent(React, usePreferences) {
  ...
  return function SessionUsage({ sessionId, useProjection }) {
    ...
    const display = exchangeAmount(total?.total || 0, sourceCurrency, locale);
    const currency = display.currency;
    const money = (value) => {
      const converted = exchangeAmount(value, sourceCurrency, locale);
      return (converted.converted ? "≈ " : converted.unavailable ? sourceCurrency + " " : "") +
          ...
          priced
            ? h(
                ...
                display.converted || display.unavailable
                  ? h("span", null, display.converted ? "≈ " : sourceCurrency + " ") : null,
                h(RollingAmount, {
                  key:
                ...
                : !data
                  ...
                  : h(
                      ...
                      exchangeNote(sourceCurrency, locale)
                        ? h("p", { className: "kj-usage-note", role: "status" }, exchangeNote(sourceCurrency, locale)) : null,
                      h(
                        "dl",
