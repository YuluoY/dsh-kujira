import { beijingParts } from "./billing.js";
...
export function createUsageComponent(React, usePreferences) {
  ...
  return function SessionUsage({ sessionId, useProjection }) {
    ...
    const missingReason = data?.skipped.length
      ...
      : "";
    const pricingNote = missingReason
    ...
    const weekend =
      data &&
      [0, 6].includes(beijingParts(data.previewTime || data.updatedAt).weekday);
    const scheduleTip =
      nextLabel &&
      [
        weekend ? t("周末全天谷价（北京时间）") : "",
        nextLabel + " · " + t("设备当地时间"),
      ]
        .filter(Boolean)
        .join("\n");
      ...
      h(
        "button",
        {
          ...
          "data-tooltip": error
            ? t(error)
            : pricingNote ||
              (!data?.hasUsage ? t(title) : scheduleTip || t(rate)),
