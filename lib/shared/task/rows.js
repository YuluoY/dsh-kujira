...
import { compactStatus, fileName } from "./status.js";
/**
        ...
        h(
          ...
          op.path || op.description
            ? h(
                ...
                fileName(op.path) || op.description,
              )
            : null,
        ),
      ...
      expanded
        ? h(
            ...
            op.name && title !== op.name
              ...
              : null,
            op.path ? h("p", { className: "kj-task-path" }, op.path) : null,
            op.result
              ? markdown(op.result)
              : h(
                  "p",
  ...
  const childRow = (child) =>
    h(
      ...
      h(
        ...
        icon(child.stage),
        h("strong", null, child.label || t("子任务")),
