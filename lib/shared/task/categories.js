...
export function createTaskCategories({
  ...
  const category = (
    ...
    items.length
      ? { id, label, count: items.length, content, attention, progress }
      : null;
  return (a, state) =>
    [
      category(
        ...
        list(
          ...
          (todo, i) =>
            h(
              ...
              h(Text, { text: todo.label, lines: 1 }),
            ),
      ...
      category(
        ...
        list("files", a.artifacts, (file) =>
          h(
            ...
            action(
              fileLabel(file.path).name,
              { kind: "file", path: file.path },
              null,
              { fullText: file.path, lines: 1, file: true },
            ) ||
              h(Text, {
                className: "kj-task-file-static",
                "data-tooltip": file.path,
                text: fileLabel(file.path).name,
                fullText: file.path,
                lines: 1,
              }),
          ),
    ...
    return h(
      ...
      ...items.map((item) =>
        h(
          ...
          action(
            markdownExcerpt(item.text, 12000),
            { kind: "message", ...item },
            null,
            {
              fullText: item.text,
              excerpted: markdownExcerpt(item.text, 12000) !== item.text,
            },
          ) ||
            h(
              "p",
