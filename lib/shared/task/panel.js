import { createTaskDetail } from "./detail.js";
...
export function createTaskPanel(React, icon) {
  ...
  const { Skeleton } = createControls(React);
  ...
  const Detail = createTaskDetail(React, Markdown);
  const memories = new Map();
  return function Panel({ state, onClose, shell }) {
    const saved = memories.get(state.sessionId);
    const a = taskPresentation(state.data);
    const [detail, setDetail] = React.useState(saved?.detail || null),
    ...
    const [tab, setTab] = React.useState(saved?.tab || null),
      [limits, setLimits] = React.useState(saved?.limits || {});
    const scrollMemory = React.useRef(saved?.scroll || {});
    ...
    latest.current = {
      ...
      messages: messageMemory.current,
    };
    React.useEffect(
      () => () => {
        if (!state.sessionId) return;
        memories.delete(state.sessionId);
        memories.set(state.sessionId, {
          ...latest.current,
          scroll: scrollMemory.current,
          messages: messageMemory.current,
        });
    ...
    const open = async (target) => {
      ...
      const stayOpen =
        target.kind === "child" ||
        (target.kind === "back" &&
          state.navigate.backTarget?.()?.kind === "child");
      setBusy(true);
      setNotice("");
      navigationAbort.current?.abort();
    ...
    const markdown = (text, target) =>
      h(a?.detailsOnDemand && target ? Detail : Markdown, {
        key: target,
        sessionId: state.sessionId,
        target,
        fallback: text,
        version:
          target === "result"
            ? a?.summaryLocation?.seq
            : target?.startsWith("child:")
              ? child?.updatedAt
              : a?.operations.find((item) => item.id === target)?.endedAt,
        text,
    ...
    const returnTarget = state.navigate?.backTarget?.();
    const returnLabel =
      returnTarget?.kind === "child" ? "返回上级会话" : "返回跳转前位置";
    const content = () => {
      if (state.loading)
      ...
      h(
        ...
        returnTarget
          ? h(
              ...
              {
                type: "button",
                className: "kj-back kj-task-return",
                disabled: busy,
                "aria-label": returnLabel,
                "data-tooltip": returnLabel,
                onClick: () => open({ kind: "back" }),
              },
              icon("back"),
            )
          : null,
...
export { createTaskBoundary } from "./boundary.js";
