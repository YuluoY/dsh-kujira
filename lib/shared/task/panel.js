...
export function createTaskPanel(React, icon) {
  ...
  return function Panel({ state, onClose, shell }) {
    ...
    const markdown = (text) =>
      h(Markdown, { text, onOpenFile: (path) => open({ kind: "file", path }) });
      ...
      h(
        ...
        a.summary && ["done", "error", "stopped"].includes(a.stage)
          ? h(
              ...
              h("p", null, markdownExcerpt(a.summary)),
    ...
    const content = () => {
      ...
      if (selectedChild)
        return h(
          ...
          markdown(selectedChild.summary || t("等待子任务进展")),
          action("打开子代理", {
            kind: "child",
            id: selectedChild.id,
      ...
      if (detail === "result")
        return h(
          "div",
          { className: "kj-task-result" },
          markdown(a.summary || t("本轮未返回文字结果")),
          a.summaryTruncated
            ? h(
                "p",
