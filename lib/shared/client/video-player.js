...
export function createVideoPlayer({
  ...
  const play = (name, options = {}) => {
    ...
    if (!options.restart && front.dataset.name === name && !front.ended && !front.paused) {
    ...
    const fit = framing(name);
    const axis = (value) => Number.isFinite(value) && value >= 0.5 && value <= 2 ? value : 1;
    next.style?.setProperty("--kj-clip-scale-x", String(axis(fit?.x)));
    next.style?.setProperty("--kj-clip-scale-y", String(axis(fit?.y)));
    next.style?.setProperty("--kj-clip-offset-y", (Number.isFinite(fit?.offsetY) && Math.abs(fit.offsetY) <= 50 ? fit.offsetY : 0) + "%");
