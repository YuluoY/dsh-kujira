...
export function renderMascot({
  ...
  const blocked = !!(panel || orbOpen || pageTimerRef.current);
  const hasTask =
    taskRuntime &&
    taskView.sessionId &&
    (taskView.loading || taskView.error || taskView.data?.startedAt);
  const taskWanted = hasTask && (taskVisible || taskFocused || urgentTask);
  const kind = blocked
    ? null
    : urgentTask && hasTask
      ? "task"
