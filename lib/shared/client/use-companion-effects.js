...
export function useCompanionEffects({
  ...
  const activeReward = React.useRef(null);
  const playMomentRef = React.useRef(playMoment);
  ...
  React.useEffect(() => {
    ...
    if (feed) {
      for (const event of feed.events || []) {
        ...
        if (
          fresh &&
          inventory.execution?.active !== 0 &&
          Date.now() - event.time < 30000
        )
  ...
  React.useEffect(() => {
    ...
    const tick = () => {
      if (document.hidden) {
        setGain(null);
        setScatter(null);
        queue.current = queue.current.filter((item) => item.kind !== "start");
        if (activeReward.current) {
          enqueueReward(queue.current, activeReward.current);
          activeReward.current = null;
          clearTimeout(finish);
        }
        return;
      }
      if (activeReward.current) return;
      const next = queue.current.shift();
      if (!next) return;
      if (next.kind === "start")
        playMomentRef.current(
          cfgRef.current.ui.featureAnimations?.session || "偷吃Token",
        );
      else {
        activeReward.current = next;
        ...
        clearTimeout(finish);
        finish = setTimeout(() => {
          if (document.hidden && activeReward.current)
            enqueueReward(queue.current, activeReward.current);
          activeReward.current = null;
          setGain(null);
        }, 3000);
  ...
  const overlay = h(
    ...
    gain
      ? h(
          ...
          h(
            "span",
            { className: "kj-reward-item", "aria-hidden": true },
            supplyIcon(h, gain.kind, { solid: true }),
          ),
          h(
            "span",
            { className: "kj-reward-count", "aria-hidden": true },
            "×" +
              number(gain.count, {
                notation: "compact",
...
export function useContextReactions(props) {
  ...
  const run = () => {
    ...
    const scene =
      event.kind === "work"
        ? ["thinking", "reading", "searching"].includes(p.taskView?.data?.stage)
          ? "thinking"
          : "longWork"
        : event.kind === "balance"
          ? event.scene
    ...
    const accepted =
      scene && p.playScene
        ? p.playScene(scene, { ambient: true, guard })
        : p.playMoment(event.animation, { ambient: true, guard });
