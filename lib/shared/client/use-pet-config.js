/**
 * @description Load behavior modules and configuration before starting mascot playback.
 * @param {object} dependencies Explicit runtime and state dependencies.
 * @returns {object} Public values for the composing component.
 */
export function usePetConfig({
  useEffect,
  injectCss,
  readStore,
  SET_KEY,
  setCity,
  ASSET_BASE,
  GROWRef,
  ARCRef,
  I18N,
  prefsRef,
  cfgRef,
  normalizeConfig,
  applyVisual,
  frontRef,
  aRef,
  setReady,
  play,
  setMeta,
  greetingFor,
  speak,
  setTip,
  timerRef,
  bubbleTimerRef,
}) {
  useEffect(() => {
    injectCss();
    let alive = true;

    const restoreCity = () => setCity(String((readStore(SET_KEY) || {}).city || ""));
    restoreCity();
    window.addEventListener("kujira:preferences", restoreCity);
    window.addEventListener("storage", restoreCity);

    (async () => {
      try {
        const mods = await Promise.all([
          import(ASSET_BASE + "/shared/growth.js"),
          import(ASSET_BASE + "/shared/arc.js"),
        ]);
        GROWRef.current = mods[0];
        ARCRef.current = mods[1];

        I18N.configure(prefsRef.current.locale);
      } catch (error) {
        GROWRef.current = null;
        ARCRef.current = null;
      }

      try {
        const res = await fetch(ASSET_BASE + "/config.json", {
          cache: "no-store",
        });
        if (!res.ok) {
          throw new Error("HTTP " + res.status);
        }
        const raw = await res.json();
        if (!alive) {
          return;
        }
        cfgRef.current = normalizeConfig(raw);
        applyVisual();
        frontRef.current = aRef.current;
        setReady(true);
        play(cfgRef.current.startAnim, { loop: false });

        let systemName = "";
        try {
          const m = await fetch(ASSET_BASE + "/meta", {
            cache: "no-store",
          }).then((r) => r.json());
          if (alive) {
            setMeta(m);
            systemName = m.systemName || "";
          }
        } catch (error) {}

        const g = greetingFor(Date.now(), systemName);
        if (g && prefsRef.current.care) {
          setTimeout(() => alive && speak(g), 900);
        }
      } catch (error) {
        if (alive) {
          setTip("无法读取配置，请刷新重试。");
        }
      }
    })();

    return () => {
      alive = false;
      window.removeEventListener("kujira:preferences", restoreCity);
      window.removeEventListener("storage", restoreCity);
      clearTimeout(timerRef.current);
      clearTimeout(bubbleTimerRef.current);
    };
  }, [applyVisual, play, speak]);
  return {};
}
