...
export function normalizePreferences(value, defaults) {
  ...
  for(const [key,allowed] of Object.entries({locale:['system','zh-CN','en-US','ko-KR','ru-RU'],theme:['system','light','dark'],motion:['system','full','reduced'],menuSizing:['fixed','auto']}))
...
export function createPreferences({
  ...
  const flush = () => {
    ...
    stored.appearance = normalizePreferences({...stored.appearance,...pendingPatch}, PREF_DEFAULTS);
    pendingPatch = {};
    writeStore(SET_KEY, stored);
  };
  function readPrefs() {
    return normalizePreferences({...(readStore(SET_KEY) || {}).appearance,...pendingPatch}, PREF_DEFAULTS);
  }
