...
const DESKTOP_ERRORS = new Set(["dsh-offline", "dsh-start-failed", "dsh-start-timeout", "port-occupied-or-plugin-old", "browser-focus-failed", "web-navigation-timeout", "web-navigation-busy"]);
/**
...
export function renderOrbs({
  ...
  const allDefs = FEATURE_REGISTRY.map((f) => ({
    key: f.key,
    label: f.key === "settings" && updateAvailable ? t("设置，有 DSH 更新") : f.label,
    icon: f.icon,
          ...
          : () => {
              ...
              try {
                Promise.resolve(
                  ...
                  const reason = String(error?.message || "").split("Error: ").at(-1);
                  speak(f.key === "desktop-open" && DESKTOP_ERRORS.has(reason) ? t(reason) : "功能打开失败，请重试");
                });
              } catch {
                speak("功能打开失败，请重试");
              }
  ...
  const orbs = cfg.ui.buttons
    ? h(
        ...
        pageDefs.map((o, i) => {
          ...
          const live = o.key === "desktop-open" && dshOnline;
          const label = live ? `${o.label}（${t("DSH 已连接")}）` : o.label;
          return h(
            "button",
            {
              key: o.key,
            ...
            o.key === "settings" && updateAvailable ? h("span", { className: "kj-update-dot", "aria-hidden": true }) : null,
            live ? h("span", { className: "kj-orb-live", "aria-hidden": true }) : null,
            o.key === "feed"
              ? h(
                  "span",
                  { className: "kj-stock-badge", "aria-hidden": true },
                  inventory?.ok && !inventory.free && inventory.stock.fish > 99
                    ? "99+"
                    : stockCount("fish"),
                )
              : null,
