...
export function useCareActions({
  ...
  const performCare = useCallback(
    async (kind) => {
      ...
      try {
        ...
        if (!busyRef.current) play(action.anim, { loop: false });
        loadGrowth();
  ...
  useEffect(() => {
    ...
    const previous = seenDrops.current;
    seenDrops.current = inventory.drops;
    if (
      previous !== null &&
      inventory.drops > previous &&
      !busyRef.current &&
      prefsRef.current.care
    )
      speak(
        t("获得 {count} 份新补给", {
          count: I18N.number(inventory.drops - previous),
        }),
      );
  }, [inventory?.drops, speak]);
  const feed = useCallback(() => performCare("fish"), [performCare]);
  const interact = useCallback((kind) => performCare(kind), [performCare]);
