 ...
 */
const pendingSlotReach = (root) =>
  (parseFloat(root.style.getPropertyValue("--kj-orb-size")) || 34) / 2 + 8;

/**
 * Decide whether a pointer press outside the mascot still lands on a radial
 * slot. While buttons fly out, their resting positions are empty, so an eager
 * click on a slot must not be read as "click outside" and collapse the menu.
 * @param {HTMLElement} root Mascot root element.
 * @param {number} x Pointer client X.
 * @param {number} y Pointer client Y.
 * @param {object} geom Radial geometry recorded by openOrb.
 * @param {number} enterMs Configured per-button fly-out duration.
 * @param {number} now Current time for testability.
 * @returns {boolean} True when the press targets a slot during the fly-out window.
 */
export function hitsPendingOrbSlot(
  root,
  x,
  y,
  geom,
  ...
  const delays = (geom?.inDelays || []).map(Number);
  const windowMs = Math.max(0, ...delays) + Number(enterMs || 285) + 120;
  if (!geom?.openedAt || now - geom.openedAt > windowMs) return false;
  const rect = root.getBoundingClientRect();
  const cx = rect.x + rect.width / 2;
  const cy = rect.y + rect.height / 2;
  const reach = pendingSlotReach(root);
  return [...root.querySelectorAll(".dsh-kujira-orb")].some((orb) => {
    const dx = parseFloat(orb.style.getPropertyValue("--kj-dx"));
    const dy = parseFloat(orb.style.getPropertyValue("--kj-dy"));
    if (!Number.isFinite(dx) || !Number.isFinite(dy)) return false;
    return Math.abs(cx + dx - x) <= reach && Math.abs(cy + dy - y) <= reach;
  });
}
...
export function usePetMenu({
  ...
  useEffect(
    () =>
      globalThis.kujiraDesktop?.onPanel(() => {
        setPanel("settings");
        setOrbOpen(false);
      }),
    [],
  ...
  const openOrb = useCallback(() => {
    ...
    const reach =
      Math.max(44, menuButtonSize(prefsRef.current, cfg.size) + 16) + 6;
    const minimumRadius =
      reach / (2 * Math.sin(Math.PI / (2 * Math.max(1, count - 1))));
    const wantedRadius = Math.max(
      layoutFor(cfg, liveViewport(), ARC),
      minimumRadius,
    );
    const orbR = ARC
      ? ARC.fitRadius(wantedRadius, liveViewport())
      : wantedRadius;

    const stepDeg = Number(arc.stepDeg) > 0 ? Number(arc.stepDeg) : 24;
    const maxSpan = Number(arc.spanDeg) > 0 ? Number(arc.spanDeg) : 96;
    const minStep =
      (2 * Math.asin(Math.min(0.95, reach / (2 * orbR))) * 180) / Math.PI;
  ...
  useEffect(
    () =>
      globalThis.kujiraDesktop?.onDismiss?.(() => {
        clearTimeout(pageTimerRef.current);
        pageTimerRef.current = 0;
        pageRef.current = 0;
        setPage(0);
  ...
  const togglePanel = useCallback(
    (which) => {
      ...
      if (
        next &&
        !busyRef.current &&
        cfgRef.current.ui.featureAnimations?.[next]
      )
        playMoment(cfgRef.current.ui.featureAnimations[next]);
      if (next === "balance") {
        loadBalance(false);
      }
      if (next === "weather") {
