...
export function createSearchSelect(React, { h, t, position }) {
  return function SearchSelect({
      ...
      const place = () => {
        ...
        const p = position(
          rect,
          {
            ...
            height: Math.min(280, Math.max(1, matches.length) * 40 + 8),
          },
    ...
    return h(
      "div",
      { ref: box, className: "kj-search-select" },
      h(
        "div",
        {
          className: "kj-search-select-control",
        ...
        h("input", {
          ...
          onBlur: (e) => {
            if (!box.current.contains(e.relatedTarget)) close();
          },
        }),
        value || query
          ? h(
