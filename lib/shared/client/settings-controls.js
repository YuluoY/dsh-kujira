...
export function createSettingsControls({
  ...
  function SchedulerSettings({ labelNode }) {
    const [data, setData] = useState(null),
      [busy, setBusy] = useState(false),
      [error, setError] = useState("");
    const editing = useRef(false), generation = useRef(0);
    useEffect(() => {
      let active = true,
        timer;
      const load = async () => {
        ...
        try {
          ...
          if (!response.ok) throw Error();
          const value = await response.json();
          if (active && !editing.current && version === generation.current) {
            setData(value);
            setError("");
          }
        } catch {
          if (active && !editing.current && version === generation.current) setError("调度服务暂不可用");
        } finally {
    ...
    return h(
      ...
      data
        ? h(
            ...
            h(
              "p",
              { className: "kj-scheduler-summary" },
              !data.available
                ? "当前宿主不支持步骤暂停"
                : data.enabled
                  ? data.rate === "offpeak" ? t("保护已开启，等待峰价时段") : t("已暂停 {count} · 等待安全暂停 {pending}", {
                      count: I18N.number(data.paused),
                      pending: I18N.number(data.pausing || 0),
                    })
                  : "调度已关闭",
            ),
            h("p", { className: "hint" }, "峰谷统一按北京时间和中国放假安排判断，周末补班仍为谷价，与语言和货币设置无关。"),
            data.calendarYearKnown === false
              ? h("p", { className: "hint", role: "status" }, "当年中国放假日历暂缺，暂按周一至周五时段判断；日历更新后自动生效。")
              : null,
            data.enabled
              ? h(
                  "p",
                  { className: "kj-scheduler-timing" },
                  data.rate === "unknown" ? t("峰谷规则无效，请检查配置") : "",
                  data.nextAt
                    ?
                        t("下次切换 {time}", {
              ...
              : null,
            data.enabled && Array.isArray(data.sessions)
              ? h(controls.SchedulerSessionList, { key: data.rate, sessions: data.sessions, openSession, upcoming: data.rate === "offpeak" })
              : null,
    ...
    return h(
      ...
      ...definitions.map(([label, key, min, max, step, help]) =>
        h(
          ...
          h(controls.NumberField, {
            label,
            value: draft[key],
            min: key === "max" ? Math.max(min, draft.min) : min,
            max: key === "min" ? Math.min(max, draft.max) : max,
            step,
            integer: key !== "amount",
            unit:
              key === "amount"
      ...
      h(
        ...
        t("按人民币设置，当前约合 {amount}", {
          amount: I18N.money(draft.amount),
        }),
      ),
      h("p", { className: "hint" }, "新规则只影响后续消费，保留现有进度和库存，不重抽历史消费。"),
      h("p", { className: "kj-save-state", role: "status" }, error || (busy ? "正在保存…" : "修改自动保存")),
      error
        ? h(
            "button",
            {
