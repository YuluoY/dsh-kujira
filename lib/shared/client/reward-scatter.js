...
export function createSupplyRing(React, { t, money, number }) {
  ...
  return function SupplyRing({ inventory, reduced }) {
    ...
    latestEnergy.current = (inventory?.drops || 0) + (inventory?.progress || 0);
    if (!animator.current) animator.current = createSupplyProgress(setVisual);
    React.useEffect(() => {
      if (inventory?.ok)
        animator.current.update(
          (inventory.drops || 0) + (inventory.progress || 0),
    ...
    const tooltip = known
      ? t("全局补给") +
        ...
        (inventory.rewardMultiplier === 2 ? " · " + t("峰价双倍补给") : "") +
        " · " +
        t("已获得 {count} 份补给", { count: number(inventory.drops) })
      : t("正在读取补给进度");
    return h(
      "button",
      {
