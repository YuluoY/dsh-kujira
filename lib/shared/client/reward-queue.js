...
export function mergeInventoryView(previous, value) {
  ...
  if (
    ...
    previous.todayCoverage?.status === value.todayCoverage?.status &&
    previous.today?.start === value.today?.start &&
    previous.today?.total === value.today?.total &&
    previous.today?.requests === value.today?.requests &&
...
/**
 * @description Share one visibility-aware inventory connection across the footer and pet.
 */
export function createInventoryFeed({
  ...
  return {
    acquire(listener, base = "/dsh-kujira") {
      const unsubscribe = subscribeInventory(listener);
      listener(getInventorySnapshot());
      let entry = connections.get(base);
      if (!entry) {
        const poller = poll(async (signal) => {
          try {
            const response = await request(base + "/inventory", {
              cache: "no-store",
              signal,
            });
            if (!response.ok) throw Error("inventory-unavailable");
            const value = await response.json();
            if (!signal.aborted && value?.ok) publishInventory(value);
          } catch (error) {
            if (!signal.aborted)
              publishInventory({
        ...
        entry = { poller, users: 0 };
        connections.set(base, entry);
      }
      entry.users++;
      let disposed = false;
      return {
        dispose() {
          ...
          unsubscribe();
          if (--entry.users === 0) {
            entry.poller.dispose();
            connections.delete(base);
          }
        },
      };
    },
    refresh(base = "/dsh-kujira") {
      return connections.get(base)?.poller.refresh();
    },
  };
}

export const inventoryFeed = createInventoryFeed();
