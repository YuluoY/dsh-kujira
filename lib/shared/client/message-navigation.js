...
/**
 * @description Capture the visible chat anchor before an explicit message jump.
 * @param {object} doc Browser document.
 * @returns {object|null} A stable anchor and its offset in the chat scroller.
 */
export function captureChatPosition(doc = globalThis.document) {
  ...
  const anchor = [...flow.querySelectorAll("[data-chat-anchor-key]")].find(
    (node) =>
      node.getClientRects().length &&
      node.getBoundingClientRect().bottom > top + 8,
  );
  if (!anchor) return null;
  return {
    key: anchor.dataset.chatAnchorKey,
    offset: anchor.getBoundingClientRect().top - top,
  };
}
/**
 * @description Restore a captured chat anchor without guessing when it is no longer present.
 * @param {object} position Captured reading location.
 * @param {object} doc Browser document.
 * @returns {boolean} Whether the original anchor was restored.
 */
export function restoreChatPosition(position, doc = globalThis.document) {
  const anchor = [
    ...(doc?.querySelectorAll?.("[data-chat-anchor-key]") || []),
  ].find((node) => node.dataset.chatAnchorKey === position?.key);
  if (!anchor || !anchor.getClientRects().length) return false;
