/**
 * @description Map the exported character area for bubble placement without scaling the video.
 * @param {object} stage Video stage rectangle.
 * @param {object} placement Optional normalized export bounds.
 * @returns {object} Character rectangle in viewport coordinates.
 */
export function animationAnchor(stage, placement) {
  if (!placement || ![placement.width, placement.height].every(n => Number.isFinite(n) && n > 0 && n <= 1) ||
    !Number.isFinite(placement.top) || placement.top < 0 || placement.top + placement.height > 1.000001) return stage;
  const width = stage.width * placement.width, height = stage.height * placement.height;
  const left = stage.left + (stage.width - width) / 2, top = stage.top + stage.height * placement.top;
...
function thoughtGeometry(anchor, size, viewport) {
  const edge = 12, trail = 63, sideTrail = 70;
  ...
  for (const side of ["left", "right"]) {
    ...
    if (viewport.w >= width + sideTrail + edge * 2)
      candidates.push({ side, dock: "side", x: side === "left" ? face.left - width - sideTrail : face.right + sideTrail,
        y: (headTop + headBottom - height) / 2, target, direction });
  }
  for (const item of candidates) {
    const sideTail = item.dock === "side";
    item.left = clamp(item.x, edge + (sideTail && item.side === "right" ? sideTrail : 0),
      viewport.w - width - edge - (sideTail && item.side === "left" ? sideTrail : 0));
    ...
    if (sideTail) {
      ...
      item.small = { x: item.side === "left" ? width + 44 : -62, y: height / 2 - 9, size: 18 };
    } else {
      item.large = { x: item.tailX - 14, y: item.dock === "above" ? height + 1 : -29, size: 28 };
      item.small = { x: item.tailX + item.direction * 20 - 9, y: item.dock === "above" ? height + 37 : -55, size: 18 };
    }
...
export function bubbleGeometry(anchor, size, viewport, thought = false) {
  ...
  const sides = [
    { side: "left", x: anchor.left + anchor.width * 0.18 - width - 8 },
    { side: "right", x: anchor.right - anchor.width * 0.18 + 8 },
  ];
  const candidates = sides.flatMap(({ side, x }) => [
    { side, x, y: anchor.top + anchor.height * 0.44 - height / 2, below: false, penalty: 0 },
    { side, x, y: head - height - trail, below: false, penalty: 20 },
    { side, x, y: anchor.bottom + trail, below: true, penalty: 40 },
  ]);
  ...
  return {
    ...
    tailY: clamp(anchor.top + anchor.height * 0.44 - best.top, 12, height - 12),
  };
}
...
export function fitBubble(speech, anchor, viewport, outline) {
  ...
  if (!thought) speech.style.width = Math.max(144, Math.min(200, anchor.width * 0.75)) + "px";
  speech.style.maxWidth =
    Math.max(1, Math.min(thought ? 244 : 200, viewport.w - 24, thought ? (viewport.h - 96) * 380 / 320 : Infinity)) + "px";
  ...
  if (shell && outline) {
    ...
    shell
      .querySelector("path")
      .setAttribute("d", outline(p.width, p.height, p.tailY ?? p.height - 16, true, true));
    shell
      .querySelector("path")
      .setAttribute(
        "transform",
