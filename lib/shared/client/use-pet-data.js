import { mergeInventoryView } from "./reward-queue.js";
/**
...
export function usePetData({
  ...
  const acceptInventory = useCallback((value) => {
    if (value?.ok) setInventory((old) => mergeInventoryView(old, value));
  }, []);
  const refreshInventory = useCallback(async () => {
    try {
      ...
      acceptInventory(await response.json());
    } catch {
      setInventory((old) => (old ? { ...old, stale: true } : { ok: false }));
    }
  }, [acceptInventory]);
  useEffect(() => {
    refreshInventory();
