import { inventoryFeed, publishInventory } from "./reward-queue.js";
/**
...
export function usePetData({
  ...
  const weatherRegion = I18N.serviceRegion(prefsRef.current.weatherRegion);
  const acceptInventory = useCallback((value) => {
    if (value?.ok) setInventory(publishInventory(value));
  }, []);
  useEffect(() => {
    const connection = inventoryFeed.acquire(setInventory, ASSET_BASE);
    return connection.dispose;
  }, [ASSET_BASE, setInventory]);
  const refreshInventory = useCallback(
    () => inventoryFeed.refresh(ASSET_BASE),
    [ASSET_BASE],
  );
  ...
  const loadWeather = useCallback(
    ...
    [city, locale, weatherRegion],
  );
  useEffect(() => {
    if (panel === "weather") loadWeather(false);
  }, [locale, weatherRegion]);
