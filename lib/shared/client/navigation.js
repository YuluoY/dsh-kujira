...
export function createNavigation({ taskRuntime, ctx, history = [] }) {
  ...
  const navigateTask = async (target) => {
    ...
    if (target.kind === "session") { await service("sessions").open(target.sessionId); return true; }
  ...
  navigateTask.canOpen = (target) => {
    if (target?.kind === "session") return typeof target.sessionId === "string" && !!target.sessionId && typeof service("sessions")?.open === "function";
