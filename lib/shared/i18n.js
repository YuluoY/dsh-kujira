...
/**
 * @description Format the independently selected display currency in the interface locale.
 ...
 */
export function money(value, currency = "CNY", target = locale, decimals = 6) {
  const result = exchangeAmount(value, currency);
  return (
    (result.converted ? "≈ " : result.unavailable ? currency + " " : "") +
    rawMoney(result.value, result.currency, target, decimals)
  );
}
export function exchangeNote(currency = "CNY", target = locale) {
  const result = exchangeAmount(1, currency);
  if (!result.converted)
    return result.unavailable
      ? t(
          exchangeSnapshot() === null
            ? "正在读取汇率，暂显示 {currency}"
            : "汇率暂不可用，显示原币种 {currency}",
          { currency },
          target,
        )
      : "";
  return t(
      ...
      : "约合 {currency} · 参考汇率 {date} · {source}（每日更新）",
    {
      ...
      date: result.date,
      source: exchangeSnapshot()?.source || "Frankfurter",
    },
    target,
  );
}
