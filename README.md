...
桌面应用可独立运行，通过人物菜单打开本机 DSH；插件安装方式不变，桌面运行环境单独安装。外观与本地养成快照在浏览器/桌面两端同步；库存、奖励及调度仍由 DSH 服务负责，离线不伪造交易。平台边界、默认配置与构建方法见[桌面模式文档](docs/DESKTOP.md)。可选桌面预览包见 [v0.2.1 Release](https://github.com/YuluoY/dsh-kujira/releases/tag/v0.2.1) 附件；这些预览包未签名。
